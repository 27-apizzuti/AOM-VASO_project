...
Prepare depth-dependent in house sens. and spec. and EPI for cvessel detection (BOLD_mean.nii)
...
for su in SUBJ:
    ...
    # FILE2: BOLDMask
    FILE2 = os.path.join(PATH_IN, "{}_{}_BOLD_FDR_mask_scaled_4.nii.gz".format(su, ROI_NAME))
    nii2 = nb.load(FILE2)
    ...
    for fu in FUNC:
        ...
        FILE = os.path.join(PATH_IN, 'winner_maps', "{}_{}_{}_sensitivity_unthreshold_fix_hd_scaled_4.nii.gz".format(su, ROI_NAME, fu))
        nii = nb.load(FILE)
        sens_map = nii.get_fdata()
        sens_map = sens_map.flatten()

        # Import specificity map
        FILE = os.path.join(PATH_IN, 'winner_maps', "{}_{}_{}_specificity_unthreshold_fix_hd_scaled_4.nii.gz".format(su, ROI_NAME, fu))
        nii = nb.load(FILE)
        ...
        # Save figure
        plt.suptitle("Vein Detection for {}, {}".format(su, fu))
        fig_filename = "Vein_detection_{}_{}".format(su, fu)
        fig.tight_layout()
