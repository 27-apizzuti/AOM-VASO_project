...
NOTE: Run separately for BOLD and VASO [FUNC input] and for each ROI (e.g 'right', 'left') [ROI_NAME]
      Multiple subject allowed.
...
for su in SUBJ:
    ...
    CVFLD = glob(os.path.join(PATH_VMP, "run*", ""))

    for cv in CVFLD:
        for fu in FUNC:
            for roi in ROI_NAME:

                print("Processing {} {} {} {}".format(fu, su, cv, roi))
                ...
                # Read VMP
                IN_FILE = os.path.join(cv, 'GLM', 'ROI', FILENAME_VMP)
