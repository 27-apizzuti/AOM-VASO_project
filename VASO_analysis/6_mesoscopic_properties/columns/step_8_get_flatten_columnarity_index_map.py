...
Patch Flattening script: columnar maps
...
for iterSbj, su in enumerate(SUBJ):
    ...
    PATH_COL_MAP = os.path.join(STUDY_PATH, su, 'derivatives', 'func', 'AOM', 'vaso_analysis', 'standard', 'columns')
    ...
    VALUES = [ os.path.join(PATH_COL_MAP, '{}_leftMT_Sphere16radius_BOLD_FDR_BOLD_columns_full_depth_UVD_columns_mode_filter_window_count_ratio.nii.gz'.format(su)),
    os.path.join(PATH_COL_MAP, '{}_leftMT_Sphere16radius_BOLD_FDR_VASO_columns_full_depth_UVD_columns_mode_filter_window_count_ratio.nii.gz'.format(su))]
