...
for j, su in enumerate(SUBJ):
    ...
    for i, fu in enumerate(FUNC):
        ...
        # axs[i, j].grid("True")
        # axs[i, j].set_title("{}, {}, {}/{} ({}%)\n".format(su, fu, n_vox_tag, n_vox_avg_cv, int(perc)))
        # fig_filename = "CV_AllSbj_Sens_depth_tag_voxel_mask_{}".format(MASKS)

        # Scatterplot Spec vs depth
        ...
        axs[i, j].grid("True")
        axs[i, j].set_title("{}, {}, {}/{} ({}%)\n".format(su, fu, n_vox_tag, n_vox_avg_cv, int(perc)))
        fig_filename = "cv_AllSbj_Spec_depth_tag_voxel_mask_{}".format(MASKS)

        # # Scatterplot EPI intensitiy vs depth
        # axs[i, j].scatter(metric[idx], EPI[idx], color='black', alpha=0.2)
        # axs[i, j].scatter(metric[idx2], EPI[idx2], color='red', alpha=0.2)
        # axs[i, j].set_xlabel("depths")
        # axs[i, j].set_ylabel("EPI intensity")
        # axs[i, j].grid("True")
        # axs[i, j].set_title("{}, {}, {}/{} ({}%)\n".format(su, fu, n_vox_tag, n_vox_avg_cv, int(perc)))
        # fig_filename = "AllSbj_EPI_depth_tag_voxel_mask_{}".format(MASKS)
