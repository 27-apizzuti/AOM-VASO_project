...
for iterSbj, su in enumerate(SUBJ):
    ...
    PATH_WM = os.path.join(STUDY_PATH, su, 'derivatives', 'func', 'AOM', 'vaso_analysis', CONDT[0], 'masks_maps', 'res_pt2', 'winner_maps')
    PATH_PSC = os.path.join(STUDY_PATH, su, 'derivatives', 'func', 'AOM', 'vaso_analysis', CONDT[0], 'masks_maps', 'res_pt2', 'psc')
    PATH_MAPS = os.path.join(STUDY_PATH, su, 'derivatives', 'func', 'AOM', 'vaso_analysis', CONDT[0], 'masks_maps', 'res_pt2')

    OUTPUT_DIR = os.path.join(STUDY_PATH, su, 'derivatives', 'func', 'AOM', 'vaso_analysis', CONDT[0], 'masks_maps', 'patch_flatten')
    if not os.path.exists(OUTPUT_DIR):
        os.mkdir(OUTPUT_DIR)
        os.mkdir(os.path.join(OUTPUT_DIR, 'maps'))
        os.mkdir(os.path.join(OUTPUT_DIR, 'masks'))

    VALUES = [
     os.path.join(PATH_MAPS, 'maps', '{}_{}_BOLD_sensitivity_map_scaled_4.nii.gz'.format(su, ROI_NAME)),
     os.path.join(PATH_MAPS, 'maps','{}_{}_VASO_sensitivity_map_scaled_4.nii.gz'.format(su, ROI_NAME)),
     os.path.join(PATH_MAPS, 'maps','{}_{}_BOLD_specificity_map_scaled_4.nii.gz'.format(su, ROI_NAME)),
     os.path.join(PATH_MAPS, 'maps','{}_{}_VASO_specificity_map_scaled_4.nii.gz'.format(su, ROI_NAME))]
