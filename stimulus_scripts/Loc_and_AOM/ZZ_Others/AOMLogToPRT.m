%% P02
mainPath = '/Volumes/Elements/AOM';
subjNames = {'P01_Pilot','P02_Pilot'};
...
plotPRT = 0; % flag to plot prt file and save it
for ss=1:numel(subjNames)
    
    subjPath = fullfile(mainPath, subjNames{ss});
    file = dir(fullfile(subjPath,'Stim','Logging','*_RndDotMot*.log')); % load log file

    for rr=1:numel(file)

        fid = fopen(fullfile(subjPath,'Stim','Logging', file(rr).name));
        ...
        if numel(condInit_all{1}) ==1
            ...
            temp = [condInit_all{3};condInit_all{4};condInit_all{5};condInit_all{6}];
            FixOnOff = [1 min(temp)];
        ...
        prtFile.Experiment = [subjNames{ss} '_AOM_run0' num2str(rr)];
        prtFile.ParametricWeights =  0;
        prtFile.FileVersion = 2;
        prtFile.SaveAs(fullfile(subjPath,[prtFile.Experiment '.prt']))
        ...
        if plotPRT % create prt plot
            ...
            exportgraphics(figH,fullfile(subjPath,[prtFile.Experiment '.png']),'Resolution',300)
        end
